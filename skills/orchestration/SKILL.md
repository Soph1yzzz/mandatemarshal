...
### v0.1 host limitation

A host may decide whether to load a Skill **before** MandateMarshal code can query the external registry. Therefore v0.1 cannot guarantee registry-driven auto-rediscovery in every brand-new host context. When pre-dispatch registry lookup is unavailable, continuation in an already-active project may rely on the current Codex/host context. This is a host-discovery limitation, not permission to auto-activate unrelated projects.
...
Default Codex mapping for v0.1:
