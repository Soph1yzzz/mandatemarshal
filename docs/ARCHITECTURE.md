...
## Skill-run receipt and authority-state boundary — v0.2.9
...
The persistent receipt under `~/.mandatemarshal/receipts/` retains only current authority/recovery facts: project/run identity, current candidate, Git HEAD locator, Parent verification binding, role/thread locators, latest reviewer verdict, Fresh PASS binding, review purpose, and generic scoped grant lifecycle. A PASS may create scopes only for the exact current Parent-verified candidate. Scope names are opaque project data; the core does not embed domain-specific actions. Current grants become historical on candidate drift, compatible runtime upgrade, or a newer PASS that explicitly re-grants the same scope. Consumption and revocation are explicit terminal states for that grant entry.

`run reconcile` re-observes candidate/HEAD before reporting authority. Optional Git-ref evidence accepts only fully-qualified `refs/...` names and uses exact plumbing; a tag or branch never creates authority by its presence alone. Human-facing prose is likewise explanatory rather than a second authority database.

Detailed event trace is diagnostic-only, lives under the OS temporary directory, and has a fixed 30-day TTL. Trace persistence and cleanup are best-effort and may disappear without invalidating the persistent receipt.

Receipt state fails closed when candidate, Parent-verification, Fresh PASS, grant lifecycle, or completion bindings are inconsistent. A `FIX` or changed candidate clears the old PASS. `run-completed` is rejected unless the current candidate has a fresh PASS.
