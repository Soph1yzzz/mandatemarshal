...
Current Codex routing is intentionally two-model and fixed:

- Parent -> GPT-6 Sol / High
- fresh reviewer -> GPT-6 Sol / High, fresh context, read-only
- default implementation -> GPT-6 Luna / Max
- explicit blocked-implementation escalation -> GPT-6 Sol / High

Every settled implementation starts on the Luna lane. Do not pre-route work to Sol merely because it looks difficult, broad, security-sensitive, or high-context. The Sol implementation lane is entered only after the Luna worker actually reports a concrete blocker and Parent explicitly records the `routine-implementer -> complex-implementer` reclassification with a non-empty reason.

The semantic lane names remain `routine-implementer` and `complex-implementer` for wire compatibility, but in v0.3.0 `complex-implementer` means the explicit Sol escalation lane, not a predictive complexity tier. Failure to launch Luna is a capability error, not a blocker that authorizes Sol substitution.

The Parent is the user-facing root Codex session, not a child that MandateMarshal should recursively respawn. When the host exposes root-session model/effort observation, require exact `gpt-6-sol/high`. When root-session observation is unavailable, treat that exact selection as a host/user precondition and never claim it was mechanically observed.

For Fresh Reviewer launch, request exact `gpt-6-sol` with `reasoning_effort=high` and read-only isolation. The packaged reviewer profile is `mandatemarshal_fresh_reviewer`. Astra profiles and the old 5.6 Sol compatibility profile are not active v0.3.0 routes.

These mappings remain adapter facts rather than core ontology, so a future model generation can roll forward without changing the Owner/Parent/Implementer/Reviewer authority model. No model, effort, role, or lane may be silently substituted.
